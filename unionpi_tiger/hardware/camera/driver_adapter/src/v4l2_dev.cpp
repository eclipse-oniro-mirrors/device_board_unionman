...
using V4L2_AWB_Mode = enum V4L2_AWB_MODE {
    V4L2_AWB_OFF = 0,
    V4L2_AWB_AUTO = 1,
    V4L2_AWB_INCANDESCEN = 2,
    V4L2_AWB_FLUORESCENT = 3,
    V4L2_AWB_WARM_FLUORESCENT = 4,
    V4L2_AWB_DAYLIGHT = 5,
    V4L2_AWB_CLOUDY_DAYLIGHT = 6,
    V4L2_AWB_TWILIGHT = 7,
    V4L2_AWB_SHADE = 8,
};
using ISP_AWB_Mode = enum ISP_AWB_MODE {
    ISP_AWB_AUTO = 0x35,
    ISP_AWB_MANUAL = 0x36,
    ISP_AWB_DAY_LIGHT = 0x37,
    ISP_AWB_CLOUDY = 0x38,
    ISP_AWB_INCANDESCENT = 0x39,
    ISP_AWB_FLOURESCENT = 0x3A,
    ISP_AWB_TWILIGHT = 0x3B,
    ISP_AWB_SHADE = 0x3C,
    ISP_AWB_WARM_FLOURESCENT = 0x3D,
};
...
int HosV4L2Dev::ConvertToDevAwbMode(int awbMode)
{
    int mode = 0;

    switch (awbMode) {
        case V4L2_AWB_OFF: /* OFF */
            mode = ISP_AWB_MANUAL;
            break;
        case V4L2_AWB_AUTO: /* AUTO */
            mode = ISP_AWB_AUTO;
            break;
        case V4L2_AWB_INCANDESCEN: /* INCANDESCEN */
            mode = ISP_AWB_INCANDESCENT;
            break;
        case V4L2_AWB_FLUORESCENT: /* FLUORESCENT */
            mode = ISP_AWB_FLOURESCENT;
            break;
        case V4L2_AWB_WARM_FLUORESCENT: /* WARM_FLUORESCENT */
            mode = ISP_AWB_WARM_FLOURESCENT;
            break;
        case V4L2_AWB_DAYLIGHT: /* DAYLIGHT */
            mode = ISP_AWB_DAY_LIGHT;
            break;
        case V4L2_AWB_CLOUDY_DAYLIGHT: /* CLOUDY_DAYLIGHT */
            mode = ISP_AWB_CLOUDY;
            break;
        case V4L2_AWB_TWILIGHT: /* TWILIGHT */
            mode = ISP_AWB_TWILIGHT;
            break;
        case V4L2_AWB_SHADE: /* SHADE */
            mode = ISP_AWB_SHADE;
            break;
        default:
            mode = ISP_AWB_AUTO;
...
int HosV4L2Dev::ConvertToHosAwbMode(int awbMode)
{
    int mode = 0;

    switch (awbMode) {
        case ISP_AWB_MANUAL:
            mode = V4L2_AWB_OFF; /* OFF */
            break;
        case ISP_AWB_AUTO:
            mode = V4L2_AWB_AUTO; /* AUTO */
            break;
        case ISP_AWB_INCANDESCENT:
            mode = V4L2_AWB_INCANDESCEN; /* INCANDESCEN */
            break;
        case ISP_AWB_FLOURESCENT:
            mode = V4L2_AWB_FLUORESCENT; /* FLUORESCENT */
            break;
        case ISP_AWB_WARM_FLOURESCENT:
            mode = V4L2_AWB_WARM_FLUORESCENT; /* WARM_FLUORESCENT */
            break;
        case ISP_AWB_DAY_LIGHT:
            mode = V4L2_AWB_DAYLIGHT; /* DAYLIGHT */
            break;
        case ISP_AWB_CLOUDY:
            mode = V4L2_AWB_CLOUDY_DAYLIGHT; /* CLOUDY_DAYLIGHT */
            break;
        case ISP_AWB_TWILIGHT:
            mode = V4L2_AWB_TWILIGHT; /* TWILIGHT */
            break;
        case ISP_AWB_SHADE:
            mode = V4L2_AWB_SHADE; /* SHADE */
            break;
        default:
            mode = V4L2_AWB_AUTO;
            break;
    ...
    while (streamNumber_ > 0) {
        ...
        for (int n = 0; nfds > 0; ++n, --nfds) {
            if ((events[n].events & EPOLLIN) && (events[n].data.fd != eventFd_)) {
                if (myBuffers_ == nullptr) {
                    CAMERA_LOGE("loopBuffers: myBuffers_ is nullptr\n");
                    return;
                }
...
RetCode HosV4L2Dev::UpdateSetting(const std::string& cameraID, AdapterCmd command, const int* args)
{
    ...
    switch (command) {
        ...
        case CMD_EXPOSURE_COMPENSATION:
            ...
            break;
        case CMD_AWB_MODE: {
            int32_t awbMode = ConvertToDevAwbMode(*(int32_t*)args);
            rc = myControl_->V4L2SetCtrl(fd, V4L2_CID_AUTO_WHITE_BALANCE, awbMode);
            break;
        }
        case CMD_FOCUS_MODE:
...
RetCode HosV4L2Dev::QuerySetting(const std::string& cameraID, unsigned int command, int* args)
{
    ...
    switch (command) {
        ...
        case CMD_AWB_MODE: {
            int32_t awbMode;
            rc = myControl_->V4L2GetCtrl(fd, V4L2_CID_AUTO_WHITE_BALANCE, awbMode);
            value = ConvertToHosAwbMode(awbMode);
            break;
        }
