...
/**
    ...
    Compared to socket buffer, there exists no tail and end pointer and tailroom as tail is rarely used in bluetooth
   stack \param List             : List structure used to list same type rtk buffer and manipulate rtk buffer like list.
...
/**
    ...
    \param [IN]     HeadRoom     <uint32_t>         : if caller knows reserved head space, set it; otherwise set 0 to
   use default value \return pointer to RTK_BUFFER if succeed, null otherwise
*/
RTK_BUFFER *RtbAllocate(IN uint32_t Length, IN uint32_t HeadRoom);

/**
    Free specified Rtk_buffer
    \param [IN]     RtkBuffer            <RTK_BUFFER*>        : buffer to free
*/
void RtbFree(IN RTK_BUFFER *RtkBuffer);

/**
    increment reference count
*/
void RtbIncreaseRefCount(IN RTK_BUFFER *RtkBuffer);
...
*/
uint8_t *RtbAddHead(IN OUT RTK_BUFFER *RtkBuffer, IN uint32_t Length);
...
*/
unsigned char RtbRemoveHead(IN OUT RTK_BUFFER *RtkBuffer, IN uint32_t Length);
...
/**
    ...
    \return  Pointer to the first byte of the extra data is returned
*/
EXTERN uint8_t *RtbAddTail(IN OUT RTK_BUFFER *RtkBuffer, IN uint32_t Length);

/**
    Remove a specified length data from the end of data buffer hold by specified rtk_buffer.
*/
EXTERN unsigned char RtbRemoveTail(IN OUT RTK_BUFFER *RtkBuffer, IN uint32_t Length);
...
*/
EXTERN void RtbQueueFree(RTB_QUEUE_HEAD *RtkQueueHead);
/**
...
*/
EXTERN void RtbQueueTail(IN OUT RTB_QUEUE_HEAD *RtkQueueHead, IN RTK_BUFFER *RtkBuffer);
...
/**
    ...
    \param [IN]            RtkBuffer                <RTK_BUFFER*>                 : Rtk buffer to add
*/
EXTERN void RtbQueueHead(IN OUT RTB_QUEUE_HEAD *RtkQueueHead, IN RTK_BUFFER *RtkBuffer);
...
*/
EXTERN RTK_BUFFER *RtbDequeueTail(IN OUT RTB_QUEUE_HEAD *RtkQueueHead);
...
/**
    ...
    \return    removed rtkbuffer if succeed, otherwise NULL
*/
EXTERN RTK_BUFFER *RtbDequeueHead(IN OUT RTB_QUEUE_HEAD *RtkQueueHead);
...
*/
EXTERN signed long RtbGetQueueLen(IN RTB_QUEUE_HEAD *RtkQueueHead);

/**
    Empty the rtkqueue.
    \param [IN OUT]     RtkQueueHead        <RTB_QUEUE_HEAD*>        : Rtk Queue
*/
EXTERN void RtbEmptyQueue(IN OUT RTB_QUEUE_HEAD *RtkQueueHead);
...
*/
EXTERN RTK_BUFFER *RtbTopQueue(IN RTB_QUEUE_HEAD *RtkQueueHead);
...
*/
EXTERN void RtbInsertBefore(IN OUT RTB_QUEUE_HEAD *RtkQueueHead, IN RTK_BUFFER *pOldRtkBuffer,
                            IN RTK_BUFFER *pNewRtkBuffer);

/**
    check whether the buffer is the last node in the queue
*/
EXTERN unsigned char RtbNodeIsLast(IN RTB_QUEUE_HEAD *RtkQueueHead, IN RTK_BUFFER *pRtkBuffer);
...
*/
EXTERN bool RtbQueueIsEmpty(IN RTB_QUEUE_HEAD *RtkQueueHead);

// annie_tmp
EXTERN unsigned char RtbCheckQueueLen(IN RTB_QUEUE_HEAD *RtkQueueHead, IN uint8_t Len);

EXTERN void RtbRemoveNode(IN OUT RTB_QUEUE_HEAD *RtkQueueHead, IN RTK_BUFFER *RtkBuffer);

EXTERN RTK_BUFFER *RtbCloneBuffer(IN RTK_BUFFER *pDataBuffer);
