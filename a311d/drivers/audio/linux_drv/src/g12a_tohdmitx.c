...
static int __tohdmitx_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret = -1;
...
int meson_g12a_tohdmitx_enable(bool enable)
{
    ...
    return __tohdmitx_update_bits(TOHDMITX_CTRL0, 1<<CTRL0_ENABLE_SHIFT, val);
}
...
int meson_g12a_tohdmitx_src_sel(enum g12a_tohdmitx_src src)
{
    ...
    ret = __tohdmitx_update_bits(TOHDMITX_CTRL0,
                    CTRL0_I2S_DAT_SEL |
                    CTRL0_I2S_LRCLK_SEL |
                    CTRL0_I2S_BCLK_SEL,
