...
static int __toacodec_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret = -1;
...
int meson_g12a_toacodec_enable(bool enable)
{
    ...
    return __toacodec_update_bits(TOACODEC_CTRL0, 1<<CTRL0_ENABLE_SHIFT, val);
}
...
int meson_g12a_toacodec_src_sel(enum g12a_toacodec_src src)
{
    ...
    ret = __toacodec_update_bits(TOACODEC_CTRL0,
