...
static int __t9015_update_bits(unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret = -1;
...
int __t9015_start(void)
{
    ...
    // set dai fmt
    val = ((g_t9015->fmt & SND_SOC_DAIFMT_MASTER_MASK) ==
                SND_SOC_DAIFMT_CBS_CFS) ? 0 : I2S_MODE;
    __t9015_update_bits(BLOCK_EN,
            I2S_MODE,
            val);
...
int meson_t9015_volume_set(unsigned int vol_left, unsigned int vol_right)
{
    ...
    return __t9015_update_bits(VOL_CTRL1, mask, val);
}
