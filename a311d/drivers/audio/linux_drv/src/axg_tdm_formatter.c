...
static int tdm_formatter_register(struct axg_tdm_formatter *formatter)
{
...
static struct axg_tdm_formatter *tdm_formatter_find(const char *name_prefix)
{
    struct axg_tdm_formatter *f;
    list_for_each_entry(f, &axg_tdm_formatter_list, list_in_all) {
...
static int tdm_formatter_enable(struct axg_tdm_formatter *formatter)
{
    ...
    ret = clk_set_phase(formatter->sclk, invert ? 0 : 180U);
    if (ret) {
        return ret;
    }
...
static void tdm_formatter_disable(struct axg_tdm_formatter *formatter)
{
...
static int tdm_formatter_attach(struct axg_tdm_formatter *formatter)
{
    struct axg_tdm_stream *ts = formatter->stream;
    int ret = 0;
    ...
    if (ts->ready) {
        ret = tdm_formatter_enable(formatter);
        if (ret) {
...
static void tdm_formatter_dettach(struct axg_tdm_formatter *formatter)
{
    struct axg_tdm_stream *ts = formatter->stream;
    ...
    tdm_formatter_disable(formatter);
}
                                        ...
                                        unsigned int offset)
{
    ...
    unsigned long mask;
    int i, j;
    unsigned int offset_tmp = offset;
    ...
    for (i = 0; i < AXG_TDM_NUM_LANES; i++) {
        ...
        for (j = find_first_bit(&mask, 32U); (j < 32U) && ch;
             j = find_next_bit(&mask, 32U, j + 1)) {
            val |= 1 << j;
            ch -= 1;
        }

        regmap_write(map, offset_tmp, val);
        offset_tmp += regmap_get_reg_stride(map);
    }
                               ...
                               struct axg_tdm_stream *ts)
{
    ...
    formatter->stream = ts;
    ret = tdm_formatter_attach(formatter);
    if (ret) {
        clk_disable_unprepare(formatter->pclk);
        return ret;
    }
...
void axg_tdm_formatter_power_down(struct axg_tdm_formatter *formatter)
{
    ...
    tdm_formatter_dettach(formatter);
    clk_disable_unprepare(formatter->pclk);
...
{
    return tdm_formatter_find(name_prefix);
}

static int tdm_formatter_clks_parse(struct axg_tdm_formatter *formatter)
{
...
int axg_tdm_formatter_probe(struct platform_device *pdev)
{
    ...
    formatter->map = devm_regmap_init_mmio(dev, devm_platform_ioremap_resource(pdev, 0),
                                           drv->regmap_cfg);
    ...
    /* Formatter clks */
    ret = tdm_formatter_clks_parse(formatter);
    if (ret) {
        dev_err(dev, "tdm_formatter_clks_parse failed: %d\n", ret);
        return ret;
    }
    ...
    tdm_formatter_register(formatter);
