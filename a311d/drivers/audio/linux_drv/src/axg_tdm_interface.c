...
static int __tdm_iface_register(struct axg_tdm_iface *iface)
{
...
static struct axg_tdm_iface *__tdm_iface_find(const char *name_prefix)
{
    struct axg_tdm_iface *f;
    list_for_each_entry(f, &axg_tdm_iface_list, list) {
...
static unsigned int __tdm_slots_total(u32 *mask)
{
                    ...
                    unsigned int slot_width)
{
    ...
    tx_slots = __tdm_slots_total(tx_mask);
    rx_slots = __tdm_slots_total(rx_mask);
...
static int __tdm_iface_set_sysclk(struct axg_tdm_iface *iface,
				    unsigned int freq)
{
    int ret = -ENOTSUPP;
...
static int __tdm_iface_set_stream(struct axg_tdm_iface *iface,
                    ...
                    struct axg_pcm_hw_params *params)
{
    ...
    if (__tdm_slots_total(ts->mask) < channels) {
...
static int __tdm_iface_set_lrclk(struct axg_tdm_iface *iface,
                    unsigned int rate)
{
...
static int __tdm_iface_set_sclk(struct axg_tdm_iface *iface, 
                    unsigned int rate)
{
	unsigned long srate;
	int ret;

	srate = iface->slots * iface->slot_width * rate;
                    ...
                    struct axg_pcm_hw_params *params)
{
    ...
    ret = __tdm_iface_set_sysclk(iface, params->rate*iface->mclk_fs);
    if (ret)
        return ret;
        
    ret = __tdm_iface_set_stream(iface, stream, params);
    if (ret)
        return ret;

    if ((iface->fmt & SND_SOC_DAIFMT_MASTER_MASK) ==
            SND_SOC_DAIFMT_CBS_CFS) {
        ret = __tdm_iface_set_sclk(iface, params->rate);
        if (ret)
            return ret;

        ret = __tdm_iface_set_lrclk(iface, params->rate);
        if (ret)
            return ret;
    }
...
static int __tdm_iface_enable(struct axg_tdm_iface *iface, bool enable)
{
    int ret = 0;
                    ...
                    enum axg_tdm_iface_stream stream)
{
    ...
    formatter = (stream == AXG_TDM_IFACE_STREAM_PLAYBACK) ?
                    iface->playback_formatter : iface->capture_formatter;

    if (formatter) {
        axg_tdm_formatter_power_down(formatter);
    }

    __tdm_iface_enable(iface, false);
                    ...
                    enum axg_tdm_iface_stream stream)
{
    ...
    ret = __tdm_iface_enable(iface, true);
    if (ret) {
        dev_info(iface->dev, "__tdm_iface_enable() failed. %s, %d\n", 
...
{
    return __tdm_iface_find(name_prefix);
}
