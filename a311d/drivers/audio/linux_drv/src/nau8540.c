...
static bool __nau8540_readable_reg(struct device *dev, unsigned int reg)
{
    switch (reg) {
...
static bool __nau8540_writeable_reg(struct device *dev, unsigned int reg)
{
    switch (reg) {
...
static bool __nau8540_volatile_reg(struct device *dev, unsigned int reg)
{
    switch (reg) {
...
static void __nau8540_powerup(struct nau8540 *nau8540)
{
    /* MICBIAS1 */
...
static void __nau8540_powerdown(struct nau8540 *nau8540)
{
    /* ADC1 */
    regmap_update_bits(nau8540->regmap, NAU8540_REG_POWER_MANAGEMENT,
            BIT(0), 0);
...
static int __nau8540_clock_check(struct nau8540 *nau8540, int rate, int osr)
{
...
static int __nau8540_set_sysclk(int clk_id)
{
    struct nau8540 *nau8540 = g_nau8540;
...
int nau8540_hw_params(unsigned int rate, unsigned int bit_width)
{
    ...
    ret = __nau8540_set_sysclk(NAU8540_CLK_DIS);
    if (ret)
        return ret;
    ...
    */
    ...
    if (__nau8540_clock_check(nau8540, rate, osr))
        return -EINVAL;
    regmap_update_bits(nau8540->regmap, NAU8540_REG_CLOCK_SRC,
...
/**
 * __nau8540_calc_fll_param - Calculate FLL parameters.
 ...
 */
static int __nau8540_calc_fll_param(unsigned int fll_in,
...
static void __nau8540_fll_apply(struct regmap *regmap,
        ...
        unsigned int freq_out)
{
    ...
    fs = freq_out / 256;
    ret = __nau8540_calc_fll_param(freq_in, fs, &fll_param);
    if (ret < 0) {
    ...
    __nau8540_fll_apply(nau8540->regmap, &fll_param);
    mdelay(2);
    regmap_update_bits(nau8540->regmap, NAU8540_REG_CLOCK_SRC,
            NAU8540_CLK_SRC_MASK, NAU8540_CLK_SRC_VCO);
...
static void __nau8540_reset_chip(struct regmap *regmap)
{
    regmap_write(regmap, NAU8540_REG_SW_RESET, 0x00);
    regmap_write(regmap, NAU8540_REG_SW_RESET, 0x00);
}

static void __nau8540_init_regs(struct nau8540 *nau8540)
{
...
static const struct regmap_config nau8540_regmap_config = {
    ...
    .readable_reg = __nau8540_readable_reg,
    .writeable_reg = __nau8540_writeable_reg,
    .volatile_reg = __nau8540_volatile_reg,
	...
	const struct i2c_device_id *id)
{
    ...
    nau8540->dev = dev;
    __nau8540_reset_chip(nau8540->regmap);
    __nau8540_init_regs(nau8540);
